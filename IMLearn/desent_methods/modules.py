...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        sig = np.exp(np.inner(X, self.weights)) + 1
        summ =  np.sum(y * (sig - 1) - np.log(sig))
        return np.asarray(- (1 / X.shape[0]) * summ)
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        first_output =self.fidelity_module_.compute_output(**kwargs)
        sec_output = self.regularization_module_.compute_output(**kwargs)
        return first_output + self.lam_ * sec_output
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        """
        self.fidelity_module_.weights_ = self.weights_ = weights
        if self.include_intercept_:
