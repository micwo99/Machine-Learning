...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "l1":
            self.coefs_ = self.solver_.fit(RegularizedModule(LogisticModule(), L1(), self.lam_, weights, self.include_intercept_), X, y)
        elif self.penalty_ == "l2":
            self.coefs_ = self.solver_.fit(RegularizedModule(LogisticModule(), L2(), self.lam_, weights, self.include_intercept_), X, y)
        else:
        ...
        #     self.coefs_ = self.solver_.fit(LogisticModule(weights), X, y)
        # else:
        #     reg_term = L1(weights) if self.penalty_ == "l1" else L2(weights)
        #     module = RegularizedModule(fidelity_module=LogisticModule(weights), regularization_module=reg_term,
        #                                lam=self.lam_, weights=weights, include_intercept=self.include_intercept_)
